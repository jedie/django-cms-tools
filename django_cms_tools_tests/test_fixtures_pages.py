...
from cms.models import Page, Title, settings
...
# Django CMS Tools
from django_cms_tools.fixture_helper.pages import CmsPageCreator, create_cms_index_pages, create_cms_plugin_page
from django_cms_tools.unittest_utils.page_mixins import CmsPageTestUtilsMixin
from django_cms_tools_test_project.test_cms_plugin.fixtures import ParentCmsPageCreator, create_testapp_cms_plugin_page
...
class ExistingCmsPageTests(BaseTestCase):
    ...
    def test_request_en(self):
        ...
        self.assertNotIn(TEMPLATE_INVALID_PREFIX,
...
class CreatePageTests(CmsPageTestUtilsMixin, BaseTestCase):
    ...
    def assert_test_pages(self):
        self.assert_page_titles(
            language_code="en", reference=["CreatePageTests() - English"])
        self.assert_page_titles(
            language_code="de", reference=["CreatePageTests() - Deutsch"])
    ...
    def assert_changed_entries(self):
        self.assert_page_templates(
            reference=["other_template.html", "other_template.html"])
...
class CreatePluginPageTests(BaseTestCase):
    ...
    Tests for plugin page generation with:
        ...
        django_cms_tools.fixture_helper.pages.create_cms_plugin_page
    """
    ...
    def test_url_resolve(self):
        ...
        existing_lang_codes = [
            language_code for language_code, lang_name in settings.LANGUAGES
        ]
    ...
    def test_plugin_view_en(self):
        self.assertResponse(
            self.client.get(
                '/en/simpletestapp-in-english/', HTTP_ACCEPT_LANGUAGE='en'),
            must_contain=(
                "<h1>Django-CMS-Tools Test Project</h1>",
    ...
    def test_plugin_view_de(self):
        self.assertResponse(
            self.client.get(
                '/de/simpletestapp-in-deutsch/', HTTP_ACCEPT_LANGUAGE='de'),
            must_contain=(
                "<h1>Django-CMS-Tools Test Project</h1>",
...
class CreateCMSPageTests(BaseTestCase):
    ...
    def test_other_parent_but_same_slug(self):
        ...
        page1, created = ParentCmsPageCreator(
            parent_page=home_page_draft).create()
        ...
        if python_slugify:
            # https://pypi.org/project/python-slugify/
            reference = [
                '/en/', '/en/parent-test/', '/en/parent-test/parent-test/',
                '/en/parent-test/parent-test/parent-test/'
            ]
        else:
            # django.template.defaultfilters.slugify used:
            reference = [
                '/en/', '/en/parent_test/', '/en/parent_test/parent_test/',
        ...
        self.assertEqual(pks, [
            home_page_draft.publisher_public.pk, page1.publisher_public.pk,
            page2.publisher_public.pk, page3.publisher_public.pk
        ])
        self.assertLess(home_page_draft.publisher_public.pk,
    ...
    def test_double_detection(self):
        ...
        page2, created = ParentCmsPageCreator(
            parent_page=home_page_draft).create()
